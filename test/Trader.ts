...
describe ("Ch3. Trader TEST", async () => {
    ...
    describe('Liquidity', async () => {
        it('addLiquidityKLAY (WHLE - KLAY LP)', async () => {
            ...
            // approve
            await whale.connect(tester).approve(trader.address, amountIn)
        ...
        it('addLiquidity (WHLE - FISH LP)', async () => {
            ...
            // approve
            await whale.connect(tester).approve(trader.address, amountInWhale)
            await fish.connect(tester).approve(trader.address, amountInFish)
        ...
        describe('after addLiquidity', async () => {
            ...
            it('removeLiquidity', async () => {
                ...
                // estimation
                // token.balance(lp.address) * (lpAmount / lp.totalSupply)
                // token.balance(lp.address) = token.balance(trader.address) - token.balance(owner.address)
                // ex
                // 5000 * (100 / 1000) = 500

                const estWhale = (await whale.balanceOf(whale.FishLp.address)).mul(liquidity).div(await whaleFishLp.totalSupply())
                const estFish = (await fish.balanceOf(whale.FishLp.address)).mul(liquidity).div(await whaleFishLp.totalSupply())
